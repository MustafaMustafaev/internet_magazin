...
async def update_category(db: Annotated[AsyncSession, Depends(get_db)], update_category: CreateCategory, category_id: int,
                          get_user: Annotated[dict, Depends(get_current_user)]):
    if get_user.get('is_admin'):
        categor = await db.scalar(select(Category).where(Category.id==category_id))
        if categor == None:
