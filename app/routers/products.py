...
async def get_all_products(db: Annotated[AsyncSession, Depends(get_db)]):
    prod = await db.scalars(select(Product).where(Product.is_active==True, Product.stock > 0))
    if prod == None:
...
async def product_by_category(db: Annotated[AsyncSession, Depends(get_db)], category_slug):
    categor = await db.scalar(select(Category).where(Category.slug == category_slug))
    if categor is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail='Category not found'
        )
    subcategor = await db.scalars(select(Category).where(Category.parent_id == categor.id))
    cat_and_subcat = [categor.id] + [i.id for i in subcategor.all()]
    product_categor = await db.scalars(select(Product).where(Product.category_id.in_(cat_and_subcat),
                                                       Product.stock > 0,
                                                       Product.is_active==True))
    return product_categor.all()
